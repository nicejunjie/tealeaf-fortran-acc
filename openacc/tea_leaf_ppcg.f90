...
!$OMP PARALLEL
!$OMP DO
!$acc parallel loop collapse(2) independent present(sd,r)
    DO k=y_min,y_max
        DO j=x_min,x_max
            sd(j, k) = r(j, k)/theta
        ENDDO
...
!$OMP PARALLEL
!$OMP DO
!$acc parallel loop collapse(2) independent present(u,r,Kx,Ky,sd) private(smvp)
    DO k=y_min,y_max
        DO j=x_min,x_max
            smvp = (1.0_8                                           &
                + ry*(Ky(j, k+1) + Ky(j, k))                        &
...
!$acc parallel loop collapse(2) independent present(p,r)  reduction(+:rro)
  DO k=y_min,y_max
    DO j=x_min,x_max
      p(j, k) = r(j, k)
      rro = rro + p(j, k)*r(j, k)
