...
!$  IF(OMP_GET_THREAD_NUM().EQ.0) THEN
      WRITE(g_out,'(a6,i7,5e15.7)')' step:',step,vol,mass,mass/vol,ie,temp
      WRITE(g_out,*)
      WRITE(g_out,"(A8,F20.15)") "result", temp
!$  ENDIF
  ENDIF
...
!$    IF(OMP_GET_THREAD_NUM().EQ.0) THEN
  ...
  ! Note that the "correct" solution is with IEEE switched on, 1 task, 1 thread, Intel compiler on Ivy Bridge
          ...
          IF(test_problem.EQ.6) qa_diff=ABS((100.0_8*(temp/97.3524569594752_8))-100.0_8)
          IF(test_problem.EQ.7) qa_diff=ABS((100.0_8*(temp/95.8382988127908_8))-100.0_8)
